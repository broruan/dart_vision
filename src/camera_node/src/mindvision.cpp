...
MindVision::MindVision(std::string mindvision_config, const std::string sn):
    ...
    int camera_index = 0;
    RCLCPP_INFO(logger, "CameraEnumerateDevice status=%d, count=%d", i_status, i_camera_counts);
    // 没有连接设备
    ...
    for (int i = 0; i < i_camera_counts; i++) {
        RCLCPP_INFO(logger, "Detected camera[%d] SN: %s", i, t_camera_enum_list[i].acSn);
    }

    if (!sn.empty()) {
        bool found_target_camera = false;
        for (int i = 0; i < i_camera_counts; i++) {
            if (std::strcmp(t_camera_enum_list[i].acSn, sn.c_str()) == 0) {
                camera_index        = i;
        ...
        if (!found_target_camera) {
            RCLCPP_ERROR(
                logger,
                "Requested SN '%s' not found."
                " Please check parameter /camera_node.sn and connected cameras.",
                sn.c_str()
            );
            i_status        = -1;
            i_camera_counts = 0;
        ...
        RCLCPP_INFO(logger, "Using requested SN '%s', index=%d", sn.c_str(), camera_index);
    } else {
        RCLCPP_WARN(logger, "Parameter 'sn' is empty, fallback to camera_index 0.");
    }
    ...
    // 让 SDK
    // 进入工作模式，开始接收来自相机发送的图像数据。如果当前相机是触发模式，则需要接收到触发帧以后才会更新图像。
    ...
    /*
        ...
        例如 CameraSetExposureTime   CameraGetExposureTime  设置/读取曝光时间
            ...
            CameraSetGamma、CameraSetConrast、CameraSetGain
       等设置图像伽马、对比度、RGB 数字增益等等。
    */
...
bool MindVision::GetFrame(std::shared_ptr<cv::Mat>& frame) {
    ...
    //在成功调用 CameraGetImageBuffer 后，必须调用 CameraReleaseImageBuffer
    //来释放获得的 buffer。 否则再次调用 CameraGetImageBuffer
    //时，程序将被挂起一直阻塞，直到其他线程中调用 CameraReleaseImageBuffer
    //来释放了 buffer
