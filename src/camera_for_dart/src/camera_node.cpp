...
CameraNode::CameraNode(const rclcpp::NodeOptions& options):
   ...
   // mindvision_->GetExposureTime(exposure_time_value);
    ...
    // thread_for_capture1_ = std::thread(std::bind(&CameraNode::GetImg, this));
    // thread_for_capture2_ = std::thread(std::bind(&CameraNode::GetImg, this));
    //thread_for_publish_.join();
    pub_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(6), 
        std::bind(&CameraNode::LoopForPublish, this)
    );
...
void CameraNode::sub_callback(const communicate_2025::msg::Autoaim::SharedPtr msg) {
    ...
    //std::cout << this->distance.load() << std::endl;
