...
CameraForCalibrate::CameraForCalibrate(const rclcpp::NodeOptions &options)
  ...
  RCLCPP_INFO(this->get_logger(), "camera_node start");
    ...
    mindvision_ = std::make_shared<MindVision>(
      ament_index_cpp::get_package_share_directory("lasercamera") +
      "/config/mindvision.config");
  mindvision_->SetExposureTime(5000);
  // 创建发布者
  image_publisher_ = this->create_publisher<sensor_msgs::msg::Image>(
      "/image_pub", rclcpp::SensorDataQoS().keep_last(2));

  frame_ = std::make_shared<cv::Mat>();
  thread_for_publish_ =
      std::thread(std::bind(&CameraForCalibrate::LoopForPublish, this));
}
...
void CameraForCalibrate::LoopForPublish() {
  while (rclcpp::ok()) {
    ...
    image_msg->step =
        static_cast<sensor_msgs::msg::Image::_step_type>(frame_->step);
